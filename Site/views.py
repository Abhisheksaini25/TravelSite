...
def trip(request):
    if request.method == "POST":
        ...
        path = os.path.join(settings.BASE_DIR, 'static', 'plans', city)
        data = open(path, "r")
        plan_string = data.read()
        plan_ls = plan_string.split('\n')
        plan_list = []
        for i in plan_ls:
            plan_list.append(i.split(','))
        plan_data = []
        for i in plan_list:
            di = {}
            di['day_number'] = int(i[0])
            di['description'] = i[1]
            di['image_url'] = i[3]
            plan_data.append(di)
        data.close()
        ...
        days = [
            ...
            {"day_number": 3, "description": "Explore Gateway of India and nearby",
             "image_url": "/static/avi/day1.jpg"},
            {"day_number": 4, "description": "Visit Elephanta Caves and Chowpatty Beach",
             "image_url": "/static/avi/day2.jpg"},
            {"day_number": 5, "description": "Explore Gateway of India and nearby",
             "image_url": "/static/avi/day1.jpg"},
            {"day_number": 6, "description": "Visit Elephanta Caves and Chowpatty Beach",
             "image_url": "/static/avi/day2.jpg"},
            # etc.
        ]
